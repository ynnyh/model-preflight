...
对任务「$ARGUMENTS」执行 model-router 航前检查，严格按 `~/.claude/skills/model-router/SKILL.md` 的流程：

1. **补齐**：任务描述缺「任务」或「完成标准」时，按 SKILL.md 的模板一轮问齐（不挤牙膏）；用户说"你定"才按假设判断并列出假设。信息不足不做判断。
2. **判断**：跑 `python ~/.claude/skills/model-router/scripts/route.py --text "<任务描述>" --env claude --human`。
